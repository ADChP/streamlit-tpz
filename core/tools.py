...
def consultas():
    ...
    if radio == 'Validación':
        ...
        if muni == 'Todos':
            query_6_2 = pd.read_sql('select mlc.paquete, m.municipio, mlc.vereda, mlc.cant_predios, mlc.area, ee.estado, u.usuario, mlc.ingreso, mlc.inicio, mlc.final, mlc.enlace_a, mlc.enlace_b, mlc.observacion, cc.nombre_paquete from mlc join d_municipio m on mlc.mlc_municipio = m.id join d_estadoentrega ee on mlc.mlc_estado = ee.id left join usuarios u on mlc.mlc_usuario = u.id join control_calidad cc on mlc.cc_id = cc.id order by municipio, estado, paquete', st.secrets.con_uri)
        else:
            query_6_2 = pd.read_sql(f"select mlc.paquete, m.municipio, mlc.vereda, mlc.cant_predios, mlc.area, ee.estado, u.usuario, mlc.ingreso, mlc.inicio, mlc.final, mlc.enlace_a, mlc.enlace_b, mlc.observacion, cc.nombre_paquete from mlc join d_municipio m on mlc.mlc_municipio = m.id join d_estadoentrega ee on mlc.mlc_estado = ee.id left join usuarios u on mlc.mlc_usuario = u.id join control_calidad cc on mlc.cc_id = cc.id where municipio = '{muni}' order by municipio, estado, paquete", st.secrets.con_uri)
        st.write(query_6_2)
